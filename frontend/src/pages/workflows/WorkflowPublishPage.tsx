...
export default function WorkflowPublishPage() {
  ...
  return (
    <Box>
      <PageHeader
        title="提交发布"
        subtitle={agent ? `工作流：${agent.name}` : ''}
        actions={
          <Button startIcon={<ArrowBack />} onClick={() => navigate('/workflows/my')} sx={{ textTransform: 'none' }}>
            返回
          </Button>
        }
      />
      ...
      <Box sx={{ display: 'flex', gap: 3, alignItems: 'flex-start' }}>
        ...
        <Box sx={{ flex: 1 }}>
          ...
          <Box sx={{ display: 'flex', gap: 2, justifyContent: 'flex-end' }}>
            <Button onClick={() => navigate('/workflows/my')} sx={{ textTransform: 'none' }}>取消</Button>
            <Button
          ...
          <Card>
            <CardContent sx={{ p: 3 }}>
              ...
              <Box sx={{ display: 'flex', flexDirection: 'column', gap: 1.5 }}>
                <CheckItem pass={!!agent?.name} label="工作流名称已设置" />
                <CheckItem pass={!!agent?.agent_type} label="智能体类型已设置" />
                <CheckItem pass={true} label="流程配置完整性" />
